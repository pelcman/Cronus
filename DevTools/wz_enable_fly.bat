...
rem Flying temporary stat (/gmmove) lets a GM fly anywhere instead of only on the 72 maps Nexon
rem flagged (天空地域, 母船, the Temple of Time flight). Players without the stat walk as before.
rem See docs/WZ_TOOLING.md.
rem
rem   wz_enable_fly.bat [<client folder>]      (default: Client\MapleStory_v186_edit)
setlocal
set "CLIENT=%~1"
...
echo done. Restart the client; /gmmove on should now fly on any map. Restore Map.wz.bak to undo.
exit /b 0
