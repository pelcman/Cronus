...
- [~] **現行スクリプトと Cosmic のスクリプトの突き合わせ**(2026-09-09 一覧化完了) — `DevTools/cosmic_gap.py` が
      ...
      **住人と仕掛け NPC 39 体**(2026-09-12、`feat/npc-townsfolk-nautilus-showa-ariant`、キュー #27): 牛乳 2180 の一式(記録に直前の牛を保存)、
      ムラト/輝く石/ゴミ箱/草原、切り株/地下鉄ゴミ箱、ショーワの銭湯とアジト案内、アリアントの商店 2 つ(JMS shops は NPC ID キー)・住人 9・
      壁/飾り棚/オアシス、花山 3(踏破報酬の鉱石は JMS の ID で確認)。見送り: ジャック 1092010(宝の地図 4220153 が JMS に無い)、
      ジョナサン 1092019(カモメのなぞなぞ 6400)、シュリンツ 1092008(デリー救出インスタンス)、ジャノ 2101010(670010200 が JMS に無い)、
      エリンの交換 NPC 5 体(4310000 ゴールドピック = GMS 限定)、花の摘み/薬草の藪 1043000/1043001(GMS の装備抽選)、怪しい男 1061016(GMS 書 ID)、
      次元の扉 1061009/亡霊 1061012(職業別の相手方面/2 人パーティー)、霊験な石2 1063013(2236 進行文字列の謎解き)、ボス猫 9120013/ファイト 9120010(GMS
      クイズ・交換)、美容院 9120101〜9120103(スタイル ID の JMS 照合が要る)、神獣 1101001(バフ)、キダン/プディン(プレイヤー NPC)。
      **`/sweep quests`**(2026-09-12、`feat/sweep-quest-scripts`): クエストスクリプトはクライアントがクエスト帳から開くため `/talk` でも
      NPC スイープでも描画できなかった。JMS Check がその側に指定する NPC の顔で開始/完了ページを順に描き、読み取り専用の身代わりキャラで
      走らせる(付与・ワープ・記録は全部捨てる)。クラッシュ痕は `sweep-progress.txt` の `quest` 行と切断時の `# crash? quest <id> <side>`。
      残る最上位:
