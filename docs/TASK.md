...
- [~] **現行スクリプトと Cosmic のスクリプトの突き合わせ**(2026-09-09 一覧化完了) — `DevTools/cosmic_gap.py` が
      ...
      **時間神殿の記憶・ドラン・秘密通路**(2026-09-10、`feat/quest-time-memories-and-magatia-dran`、キュー #18): 3523〜3539 は JMS 3507 の
      完了条件(infoNumber 7081 = "1")どおり記録 7081 に印、3108/3714/3953 単発、3320/3353(研究室 926120200 へ)/3321/3354、3360 は
      キーを記録 3360 に保存 → 扉 NPC 2111024(JMS secretNPC)が照合して "1" に(JMS の完了条件 infoex=1)、ポータル secret00(JMS secretDoor)。
      ドラン 2111002 の退出は [DEV]。見送り: 3933(アリアント 926000000 の分身戦インスタンス)、3941/3314/3514(バフ元・変身判定)。
      **転職 API と シグナス騎士団の転職クエスト**(2026-09-10、`feat/job-change-api-and-cygnus-knight-quests`、キュー #19):
      `player.changeJob` = JMSv186 `MapleCharacter.changeJob` の移植(`ChannelHandler.Jobs.cs`: 転職 SP、職業別 HP/MP 加算表、全回復、
      StatChanged、UserEffect JobChanged(11) の周囲通知)。オラクルの `resetStatsByJob` は毎回の転職でステータスを初期化するが、これは
      プライベートサーバー由来と判断して別 API `resetStatsForJob` にし、Cosmic が resetStats を呼ぶシグナス/アランの 1次転職だけで呼ぶ。
      **未移植**: `baseSkills`(3次以上到達時に 4次スキルのマスターレベル記録を作る) — 4次転職を実装するときに entry-blob の
      master-level 不変条件と合わせて検討。クエスト: 20100/2010x(初期武器は Cosmic の GMS 初期装備、JMS に実在確認済み)、20200/2020x
      (証票 30 枚、SP 使い残し判定 (Lv-30)*3)、2031x((Lv-70)*3)、20400〜20408(X11→X12)、20520/20600/20610/20700/20710/20720。
      勲章はすべて称号クエスト(29906〜29909)側。団長 NPC の直接転職は [DEV] 近道として残置。見送り: 20000〜20020 のエレヴ
      チュートリアル(guideHint / showIntro / 演出マップ 913040100 などクライアント演出パケットが未実装)、20522/20526/20527(騎乗ペット
      ミミアナの育成)、20016/20017(チュートリアル内)。
      **アランの転職クエスト**(2026-09-10、`feat/quest-aran-job-advancement`、キュー #20): 21015〜21018(受注のみ)、21100/21101
      (2000→2100 + resetStatsForJob)、21200/21202/21201(→2110)、21300〜21303(→2111)、21600。JMS Check の隠し記録 21203(infoNumber)は
      21200 完了で "0"、21301 完了で "1" を立てる(設定元は Check からの推定)。見送り: 21401(マッハ暴走戦 MahaBattle インスタンス)、
      21604/21613/21618(騎乗スキル teachSkill・Aran_3rdmount インスタンス)、21000〜21013(リエン導入の演出・チュートリアルマップ)、21700 系(物語 27 件、
      多くがワープ/インスタンス)。
      **teachSkill と アランの物語**(2026-09-10、`feat/teach-skill-api-and-aran-storyline`、キュー #21): `player.teachSkill(id, level)` /
      `getSkillLevel` (ChangeSkillRecordResult、マスターレベルは `NeedsMasterLevel` の派生値)。アランの物語 22 件(21700/21703/21704/21712/21716/
      21720/21729/21734/21735/21736/21738/21739/21740/21741/21748/21749/21750/21753/21754/21757/21766/21767)、思い出すスキルは Lv1 付与 [DEV]
      (Cosmic はマスターレベル付与 = GMS; JMS v186 のアラン 1〜3次スキルはマスターレベルを持たない)。メイカー 6033/6036(Lv2/3)。
      見送り: 21719(人形使いの洞窟 910510200: 出口の無い演出マップ)、21733(リス港防衛イベント、infoex 21762)、21742(墨汁復元 infoex 21763=800)、
      21746/21747(武陵道場特別層 925040001 インスタンス)、4647(ペットスキル・キャッシュ品)。
      **ミニダンジョン入口**(2026-09-10、`feat/mini-dungeon-portals`、キュー #22): 11 の `MD_*` ポータル(`ChannelHandler.MiniDungeons.cs`、
      `player.enterMiniDungeon` / `player.message` = BroadcastNotice)。JMS の各ダンジョンは本物 1 マップ + 空のコピー枠(19〜40)で、
      本物だけにモブと out00 がある(Cosmic の MD_roundTable は JMS では空の 240020512 を指すので 240020501 に修正)。
      ...
      **リアクター 18 個**(2026-09-12、`feat/reactor-spawns-and-traps`、キュー #25): Cosmic の 235 件のうちドロップだけの物はデータ経路で不要、
      副作用のある 134 件から PQ/イベントインスタンス・weakenAreaBoss(エリアボス弱体化 API 未実装: 2119000〜2119006/2229009/2619003〜2619005)・
      GMS と ID が違う昭和の迷路(8091000〜8098000: JMS の 9400209〜 は「メイプルの実3」)・バトの部屋(1200000/1209000, クエスト 6400)を除いた
      18 件。湧き位置はリアクター座標ではなく壊した人の足元(`spawnMob` の仕様)。
      **マガティア/ルディ/オルビス/エルナスの仕掛け・案内 NPC 24 体**(2026-09-12、`feat/npc-magatia-ludibrium-orbis-objects`、キュー #26):
      一言 11 体、カソン/ヒュモノイドA のクエストワープ、失踪した錬金術師の家の壁・額縁・机、アルカドノの本棚、エオス石 2 つ、ビシャスプラント、
      4次転職の試練の 紅葉玉/古代氷石/生命の泉、アルケスタ(3035 後の販売、個数選択 [DEV])。見送り: 本棚 2111012/秘密本 2111016(Cosmic は何もしない)、
      パイプ 2111017〜2111019・魔法陣 2111020〜2111023・隠された文書 2112016(記録の数値進行と NPC オブジェクト ID による謎解き)、
      ウィーバー/ネル 2040032/2040033(ペットの親密度 API 未実装)、司書ウィズ 2040052(本の受注は要 JMS 照合)、
      ジロクン/ペイ 2040020/2040021・ネーブ 2010003・モス 2080000・チエル 2090004(製作の大表)、チャーリー軍曹 2010000(交換イベント)、
      アドビス 2030008(ジャクム遠征)、コルバ 2082003(変身)、海の魔女カルタ 2060100(次元の亀裂)、バイキン 9000009(GMS イベント)。
      残る最上位:
